...
- A user-friendly CLI
- A "[revset](../reference/revsets.mdx)" language for selecting revisions
...
- Flexible customization of output using [templates](../reference/templates.mdx)
...
- **Working copy:** When using Sapling (like most VCSs), the
  ...
  is [automatically snapshotted by every command](../concepts/working-copy.mdx). New files
...
- **Conflicts:** Like most VCSs, Sapling requires the user to
  ...
  you [commit conflicts](../concepts/conflicts.mdx). Note that it's a representation of the
...
- **Undo:** jj's undo is powered by [the operation log](../concepts/operation-log.mdx), which
