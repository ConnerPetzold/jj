...
[the Git-compatibility doc](./git-compatibility.mdx) for information about how
the `jj` command interoperates with Git repos. See
[the Git command table](./git-command-table.mdx) for a table of similar commands.
...
[table](./git-command-table.mdx) explaining how to achieve various use cases.
...
- **The working copy is automatically committed.** That results in a simpler and
  ...
  commit. [Details](../concepts/working-copy.mdx).
...
- **No need for branch names (but they are supported as
  [bookmarks](../concepts/glossary.mdx#bookmark)).** Git lets you check out a commit without
...
- **Conflicts can be committed.** No commands fail because of merge conflicts.
  ...
  [Details](../concepts/conflicts.mdx).
...
- **Bookmarks/branches are identified by their names (across remotes).** For
  ...
  [Details](../concepts/bookmarks.mdx).
- **The operation log replaces reflogs.** The operation log is similar to
  ...
  powers e.g. the undo functionality. [Details](../concepts/operation-log.mdx).
