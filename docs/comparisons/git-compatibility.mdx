...
- **Branches: Yes.** You can read more about
  [how branches work in Jujutsu](../concepts/bookmarks.mdx)
...
- **Signed commits: Yes.**
  You can sign commits automatically [by configuration](config#commit-signing),
...
- Interleaving `jj` and `git` commands increases the chance of confusing branch
  ...
  ids](glossary#divergent-change). These never lose data, but can be
  annoying.
...
- Colocated repositories are less resilient to
  [concurrency](technical/concurrency#syncing-with-rsync-nfs-dropbox-etc)
