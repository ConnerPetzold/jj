...
<Callout>
  This tutorial has become somewhat out of date. Many people find the
  alternative (not quite finished) [tutorial by Steve
...
[install and configure Jujutsu](/getting-started/install-and-setup).
...
<Callout>
  **Git users:** The commit ID/hash is what you're used to from Git and should
  ...
  checkout of the repository. The change ID however, is a new concept, unique to
  Jujutsu.
</Callout>
...
of revisions to list. The flag accepts a ["revset"](/reference/revsets), which is an
...
[the revset documentation](/reference/revsets) for all revset operators and functions.

<Callout>
  If the default `jj log` omits some commits you expect to see, you can always
  run `jj log -r ::` (or, equivalently, `jj log -r 'all()'`) to see all the
  commits.
</Callout>
...
squash -i --tool meld-3`. You can configure the default with the
    [`ui.diff-editor` option](/reference/config#editing-diffs); those docs also explain how
