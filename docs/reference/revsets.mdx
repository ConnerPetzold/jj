...
Most revsets search only the [visible commits](glossary#visible-commits).
...
[divergent-change]: glossary#divergent-change
[string-literals]: templates#string-literals
...
- `trunk()`: Resolves to the head commit for the default bookmark of the default
  ...
  You can [override](/reference/config) this as appropriate. If you do, make sure it
...
- `immutable_heads()`: Resolves to
  ...
  required. See [here](config#set-of-immutable-commits) for details.
