...
- `jj undo` is now _sequential_: invoking it multiple times in sequence
  repeatedly undoes actions in the operation log. Previously, `jj undo` would
  only undo _the most recent_ operation in the operation log. As a result, a new
...
- Add a `.parents()` method to the
  [`Operation`](./reference/templates.mdx#operation-type) type in the templating
  language.
...
- Revset expressions like `hidden_id | description(x)` now [search the specified
  hidden revision and its ancestors](/reference/revsets.md#hidden-revisions) as well
...
- `jj log`/`op log` now applies `-n`/`--limit` _before_ the items are reversed.
